...
- The quiz has Math (Arithmetic and Algebra) and English (Grammar and Spelling) modes. Non-arithmetic questions are maintained from `questions.json`, copied into the protected `quiz_questions` table by `setup.sql`, and validated server-side so the browser cannot award points by itself. Keep `questions.json` out of the public web deployment because it contains the answer key; the app does not load it at runtime.
...
- The **bell icon** in the quiz top bar opens the **System Notice** modal — an alternating timeline of system announcements (each entry has an emoji, bold title, description with links, and a relative timestamp). The feed is stored in the `system_notices` table in Supabase (no redeploy needed to add or edit notices); unread entries get a coral node and the bell shows a red dot until the modal is closed. The single **Close Notice** button (or the ✕, Escape, or the dimmed backdrop) marks everything read. To publish a notice, insert a row with a higher `id` than the newest one via the Supabase Dashboard → Table Editor.
